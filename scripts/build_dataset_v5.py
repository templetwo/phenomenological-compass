...
build_dataset_v5.py
===================
Merge v0.4 training data + all supplement files into data/training_v5/.
Deduplicates by question text, stratifies by signal, reports source stats.

Usage:
    python3 scripts/build_dataset_v5.py

Inputs:
    data/training_v4/train.jsonl          (v0.4 base - 44 examples)
    data/training_v4/valid.jsonl          (v0.4 base - 11 examples)
    data/supplements/*.jsonl              (new examples from any source)

Outputs:
    data/training_v5/train.jsonl
    data/training_v5/valid.jsonl
"""
...
V4_TRAIN = BASE_DIR / "data/training_v4/train.jsonl"
V4_VALID = BASE_DIR / "data/training_v4/valid.jsonl"
SUPPLEMENTS_DIR = BASE_DIR / "data/supplements"
OUT_DIR = BASE_DIR / "data/training_v5"
...
print("Loading v0.4 base data...")
for path in [V4_TRAIN, V4_VALID]:
    with open(path) as f:
        for line in f:
            rec = json.loads(line)
...
source_stats["v0.4_base"] = len(all_examples)
print(f"  v0.4 base: {len(all_examples)} unique examples")
